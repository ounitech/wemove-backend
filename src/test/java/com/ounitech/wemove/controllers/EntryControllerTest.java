...
class EntryControllerTest {
    ...
    private static final String API_ENTRIES_URL = "/api/entries";
    ...
    void findAllEntriesTest() throws Exception {
        ...
        entry1.setId(1);
        entry1.setEntryTime(parse("2024-07-30T16:58:00"));
        entry1.setLeaveTime(parse("2024-07-30T17:58:00"));

        Entry entry2 = new Entry();
        entry2.setId(2);
        entry2.setEntryTime(parse("2025-07-30T10:00:00"));
        entry2.setLeaveTime(parse("2025-07-30T12:00:00"));

        when(entryService.findAllEntries())
                .thenReturn(List.of(entry1, entry2));

        //When //Then
        mockMvc.perform(get(API_ENTRIES_URL)
    ...
    void createEntryTest() throws Exception {
        ...
        savedEntry.setId(1);
        savedEntry.setEntryTime(parse("2024-07-30T16:58:00"));
        savedEntry.setLeaveTime(parse("2024-07-30T17:58:00"));

        Mockito.when(entryService.createEntry(any(Entry.class)))
        ...
        //When and Then
        mockMvc.perform(post(API_ENTRIES_URL)
    ...
    void findEntriesByMemberTest() throws Exception {
        ...
        when(entryService.findEntriesByMember(any(Integer.class)))
                .thenReturn(List.of(entry1, entry2));

        //When and Then
        mockMvc.perform(get(API_ENTRIES_URL+"/member/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(1))
                .andExpect(jsonPath("$[0].entryTime").value("2024-07-30T16:58:00"))
